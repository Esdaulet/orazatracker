...
router.get(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      for (const date of ramadanDates) {
        ...
        for (const [catId, count] of Object.entries(dayProgress)) {
          ...
          if (Array.isArray(count)) {
            categoryTotals[catId] += (count as number[]).reduce(
              (a, b) => a + b,
              0,
            );
          } else {
      ...
      // Build category summary (include deleted categories using NOMINATION_NAMES as fallback)
      const categorySummary = Object.entries(categoryTotals)
        .map(([catId, total]) => {
          const cat = categories[catId] as any;
          const name = cat?.name || NOMINATION_NAMES[catId] || catId;
          return {
            id: catId,
            name,
            meaning: cat?.meaning || "",
      ...
      const [allProgressSnap, usersSnap, referralsSnap, allQuizSnap] =
        await Promise.all([
          db.ref("progress").get(),
          db.ref("users").get(),
      ...
      for (const [uid, userProgressData] of Object.entries(allProgress)) {
        ...
        for (const date of ramadanDatesAll) {
          ...
          for (const [catId, count] of Object.entries(dayProg)) {
            ...
            if (!userCategoryTotals[uid][catId])
              userCategoryTotals[uid][catId] = 0;
            if (Array.isArray(count)) {
              userCategoryTotals[uid][catId] += (count as number[]).reduce(
                (a, b) => a + b,
                0,
              );
            } else {
      ...
      const categoryNominations: Record<
        string,
      ...
      for (const [uid, catTotals] of Object.entries(userCategoryTotals)) {
        ...
        for (const [catId, total] of Object.entries(catTotals)) {
          if (
            !categoryNominations[catId] ||
            total > categoryNominations[catId].total
          ) {
      ...
      const nominations = Object.entries(categoryNominations)
        .map(([catId, winner]) => {
          const cat = categories[catId] as any;
          if (!cat && !NOMINATION_NAMES[catId]) return null;
          return {
            categoryId: catId,
            categoryName: NOMINATION_NAMES[catId] || cat?.name || catId,
            meaning: cat?.meaning || "",
